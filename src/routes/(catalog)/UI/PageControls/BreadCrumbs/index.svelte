...
<div class="inline-flex space-x-0 whitespace-pre">
    <span class="hidden sm:block rounded-full bg-gray-50/50 mr-2 px-4 py-0.5 text-xs font-semibold leading-6 text-gray-600 ring-1 ring-inset ring-sky-400/10">
      Каталог
    </span>
    ...
    <span class="inline-flex items-center space-x-0 text-xs font-medium leading-6 text-gray-600">
      ...
      <!--  Родитель  -->
      {#if parentable.slug}
      <a href={`/{$grandParent?.slug}/{$parent.slug}/{$parentable.slug}`} class="flex hover:text-sky-700">
        <span>{grandParent.value}</span>
        <!-- <ChevronRightIcon aria-hidden="true" class="size-6 text-gray-400" /> -->
    </a>
