		...
		>
			...
			<div class="mx-auto max-w-sm sm:flex sm:items-center md:mt-4 lg:flex-col">
				<button
					...
					class="mb-3 flex w-full items-center justify-center rounded-lg border bg-white px-5 py-2.5 text-sm font-medium focus:z-10 focus:outline-none focus:ring-4 {favorites.current.some(
						(i) => i.id === item.id
					)
				...
				>
					<svg
						class="mr-1 h-5 w-5 {favorites.current.some((i) => i.id === item.id)
							? 'fill-red-700'
							: 'fill-none'}"
						aria-hidden="true"
						xmlns="http://www.w3.org/2000/svg"
