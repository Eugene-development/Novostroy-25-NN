    ...
    {#each data.product as item (item.slug)}
      <div class="space-y-4 rounded-lg border border-gray-100 bg-gray-50/10 p-6 shadow-sm">
        ...
        <button type="button" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 hover:underline hover:text-sky-700">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 text-gray-700 hover:text-sky-700">
        ...
        <div class="mt-4 md:mt-6 sm:flex sm:items-center lg:flex-col max-w-sm">
          <!-- <FavoriteButton product={item} /> -->

          <a href="/{data.parentable.parentable.slug}/{data.parentable.slug}/{data.slug}/{item.slug}" class="inline-flex mt-4 w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-sky-600 to-blue-700 hover:from-blue-600 hover:to-sky-500 px-5 py-2.5 text-sm font-medium text-white focus:outline-none focus:ring-4 focus:ring-sky-300">
            Подробнее
          </a>
        </div>
      </div>
